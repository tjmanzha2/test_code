...
# IR_sensor pins
pin_ir_left =       # IR_Sensor Left
pin_ir_middle =       # IR_Sensor Middle
pin_ir_right =       # IR_Sensor Right
...
# Set IR_Sensor pins as INPUT
GPIO.setup(pin_ir_left, GPIO.IN)
GPIO.setup(pin_ir_middle, GPIO.IN)
GPIO.setup(pin_ir_right, GPIO.IN)
...
pwm1.start(50)				# Start the Motor (Number in parenthesis is percentage [0-100%])
PWM1.stop()                 # Stop the motor

#Example
when Motor_A forward : pwm1.start(x)
                       pwm2.stop()
when Motor_A backward : pwm1.stop()
                        pwm2.start()
"""
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if(RC_CAR.ultra_detect <= 50):
                RC_CAR.STOP()
            else:
                if(RC_CAR.ir_detect(pin_ir_left) == false):
                    RC_CAR.right()
                if(RC_CAR.ir_detect(pin_ir_right) == True):
                    RC_CAR.left()
                RC_CAR.forward()
            """
